    ...
    function callback(results, status) {
        if (status == google.maps.places.PlacesServiceStatus.OK) {
            for (var i = 0; i < results.length; i++) {
                ...
                //var iwondow = new google.maps.InfoWindow();
                //iwondow.setPosition(results[i])
                //iwondow.setContent('<div><strong>' + results[i] + '</strong><br>' + document.getElementById('pac-input').value);
                //iwondow.open(map, marker);
            }
            alert(results)
        }
    }
